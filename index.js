...
app.post('/api/blogposts', authenticateUser, (req, res) => {
    ...
    BlogPost.find(query).populate("author") 
    .then(posts => res.json(posts))
    .catch(err => res.status(400).json(err));
    });
    
    app.get('/api/blogposts/:id', (req, res) => {
    BlogPost.findById(req.params.id).populate("author") 
...
app.post('/api/comments', authenticateUser, (req, res) => {
    ...
    Comment.find().populate("blogPost") 
    .then(comments => res.json(comments))
    .catch(err => res.status(400).json(err));
    });
    
    app.get('/api/comments/:id', (req, res) => {
    Comment.findById(req.params.id).populate("blogPost") 
    .then(comment => res.json(comment))
    .catch(err => res.status(400).json(err));
    });
